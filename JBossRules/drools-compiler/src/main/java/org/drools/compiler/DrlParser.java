...
public class DrlParser {
    ...
    private RuleParser getParser(final String text) {
        return new RuleParser( fileName, new CommonTokenStream( new RuleParserLexer( new ANTLRStringStream( text ) ) ) );
    }

    /** MODIFIED CHINMAY RITU */
    public PackageDescr parse(final Reader reader) throws IOException, DroolsParserException {
    	return parse(reader, (String)null);
    }
    
    /** Parse and build a rule package from a DRL source */
    public PackageDescr parse(final Reader reader, String fileName) throws IOException,
