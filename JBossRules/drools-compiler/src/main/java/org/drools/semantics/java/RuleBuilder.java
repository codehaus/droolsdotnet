...
public class RuleBuilder {
    ...
    private static StringTemplateGroup  ruleGroup  = null;
    private static StringTemplateGroup  invokerGroup         = null;

    private static StringTemplateGroup  _java_ruleGroup  = new StringTemplateGroup( new InputStreamReader( RuleBuilder.class.getResourceAsStream( "javaRule.stg" ) ),
            AngleBracketTemplateLexer.class );
    private static StringTemplateGroup  _java_invokerGroup = new StringTemplateGroup( new InputStreamReader( RuleBuilder.class.getResourceAsStream( "javaInvokers.stg" ) ),
            AngleBracketTemplateLexer.class );
    private static StringTemplateGroup  _csharp_ruleGroup  = new StringTemplateGroup( new InputStreamReader( RuleBuilder.class.getResourceAsStream( "csharpRule.stg" ) ),
		       AngleBracketTemplateLexer.class );
    private static StringTemplateGroup  _csharp_invokerGroup = new StringTemplateGroup( new InputStreamReader( RuleBuilder.class.getResourceAsStream( "csharpInvokers.stg" ) ),
            AngleBracketTemplateLexer.class );
    ...
    public RuleBuilder(TypeResolver resolver, ClassFieldExtractorCache cache) {
        ...
        if(DroolsEnv.isJava())
        {
        	ruleGroup = _java_ruleGroup;
        	invokerGroup = _java_invokerGroup;
        }
        else
        {
        	ruleGroup = _csharp_ruleGroup;
        	invokerGroup = _csharp_invokerGroup;
        }
    }
        ...
        for ( final Iterator it = ruleDescr.getLhs().getDescrs().iterator(); it.hasNext(); ) {
            ...
            if ( obj instanceof ConditionalElementDescr ) {
                if ( obj instanceof AndDescr ) {
                    ...
                    final EvalCondition eval = build( (EvalDescr) obj );
                    if ( eval != null ) {
                        this.rule.addPattern( eval );
                    }
                }
            } else if ( obj instanceof ColumnDescr ) {
                final Column column = build( (ColumnDescr) obj );
                if ( column != null ) {
    ...
    private void build(final Column column,
        ...
        if ( literalDescr.isStaticFieldValue() ) {
            ...
            try {
            	...
            	{
            		Class cl = Class.forName("cli.org.drools.dotnet.semantics.DotnetFieldFactory");
            		Method method = cl.getMethod("getFieldValueForStaticField", new Class[]{Class.forName("java.lang.String"), Class.forName("java.lang.String"), Class.forName("org.drools.spi.TypeResolver")});
            		field = (FieldValue)method.invoke(null, new Object[]{className, fieldName, this.typeResolver});
            ...
            try {
            	...
            	{
            		Class cl = Class.forName("cli.org.drools.dotnet.semantics.DotnetFieldFactory");
            		Method method = cl.getMethod("getFieldValue", new Class[]{Class.forName("java.lang.String"), Class.forName("java.lang.Integer")});
            		field = (FieldValue)method.invoke(null, new Object[]{literalDescr.getText(), new Integer(extractor.getObjectType().getValueType())});
    ...
    private void buildConsequence(final RuleDescr ruleDescr) {
        ...
        /* MODIFIED BY CHINMAY RITU */
        if(!DroolsEnv.isJava())
        {
            st.setAttribute("FileName", ruleDescr.getFileName());
            st.setAttribute("LineStart", ruleDescr.getConsequenceLine());
        }
    ...
    private void buildRule(final RuleDescr ruleDescr) {
        ...
        if(DroolsEnv.isJava())
        {
			...
			{
				String usingName = it.next().toString();
			    buffer.append( "import " + usingName + ";" + lineSeparator );
			}
        ...
        {
        	buffer.append( "namespace " + this.pkg.getName() + lineSeparator +"{" + lineSeparator);
			for ( final Iterator it = this.pkg.getImports().iterator(); it.hasNext(); ) 
			{
				String usingName = it.next().toString();
				int index = usingName.indexOf(".*");
				String newName = usingName;
				if(index >0)
				{
					newName = usingName.substring(0,index);	
				}
			    buffer.append( "using " + newName + ";" + lineSeparator );
			}
			buffer.append("[System.Serializable]" + lineSeparator);
			buffer.append( "public class " + ucFirst( this.ruleDescr.getClassName() ) + " {" + lineSeparator );
        }
    ...
    private Evaluator getEvaluator(final PatternDescr descr,
    	...
    	Evaluator eval = null; 
    	if(DroolsEnv.isJava())
    		eval = EvaluatorFactory.getEvaluator( valueType,
                                                                   evaluatorString );
    	else
    	{
    		try
    		{
	    		Class cl = Class.forName("cli.org.drools.dotnet.semantics.DotnetEvaluatorFactory");
	    		Method method = cl.getMethod("getEvaluator", new Class[]{Class.forName("java.lang.Integer"), Class.forName("java.lang.String")});
	    		eval = (Evaluator)method.invoke(null, new Object[]{new Integer(valueType), evaluatorString});
    		}
    		catch(Exception e)
    		{
