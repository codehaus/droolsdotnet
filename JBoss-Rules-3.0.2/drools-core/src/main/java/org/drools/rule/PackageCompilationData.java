...
public class PackageCompilationData
    implements
    Externalizable //RItu changed to from externizable to Serializable 
    {
    ...
    protected Map                          assemblySerializationInfoStore            = new HashMap();
    ...
    public void writeExternal(final ObjectOutput stream) throws IOException {
    	if(DroolsEnv.isJava())
    		...
    		stream.writeObject( this.assemblySerializationInfoStore );
        stream.writeObject( this.AST );

        // Rules must be restored by an ObjectInputStream that can resolve using a given ClassLoader to handle seaprately by storing as
        // a byte[]        
        final ByteArrayOutputStream bos = new ByteArrayOutputStream();
        ObjectOutput outTemp = null;
        if(DroolsEnv.isJava())
        {
        	outTemp = new ObjectOutputStream( bos );
        }
        else
        {
        	try
        	{
	        	Class clazz = Class.forName("cli.org.drools.dotnet.util.DotnetObjectOutputStream");
	        	Constructor constructor = clazz.getConstructor(new Class[]{java.io.OutputStream.class});
	        	outTemp = (ObjectOutputStream)(constructor.newInstance(new Object[]{bos}));
        	}
        	catch(Exception e)
        	{
        ...
        final ObjectOutput out = outTemp;
    ...
    public void readExternal(final ObjectInput stream) throws IOException,
        ...
        else
        {
        	...
        	this.assemblySerializationInfoStore = (Map) obj;
        	this.loadAssemblies();
        }
    ...
    public byte[] read(final String resourceName) {
        ...
        if ( this.store != null ) {
        	if(DroolsEnv.isJava())
        		...
        		return (byte[]) this.assemblySerializationInfoStore.get( resourceName.replace( '.',
                '/' ) + ".class" );
        }
        return bytes;
    }
    ...
    public void write(final String resourceName,
                      final Object clazzData) throws RuntimeDroolsException {
    	
    	Object resObj = null;
    	if(DroolsEnv.isJava())
    	{
    		resObj = this.store.put( resourceName.replace( '.',
            '/' ) + ".class",clazzData );
    	}
    	else
    	{
    		if (clazzData instanceof byte[]) {
    			this.assemblySerializationInfoStore.put(resourceName.replace( '.',
	            '/' ) + ".class", clazzData);
			}
    		else
    		{
	    		this.store.put( resourceName.replace( '.',
	            '/' ) + ".class",((Object[])clazzData)[0] );
	    		this.assemblySerializationInfoStore.put(resourceName.replace( '.',
	            '/' ) + ".class",((Object[])clazzData)[1]);
    		}
    	}
		if (  resObj != null ) { // we are updating an existing class so reload();
			reload();
		} else {
        try {
            wire( resourceName );
